...
class GoogleCalendar(Google):
    ...
    def add_event(
        self, event_id, summary, start_time, end_time, location="", description=""
    ):
        """
        ...
        event = {
            ...
            "end": {"dateTime": end_time.isoformat(), "timeZone": self.context.tz.zone},
            "reminders": {
                "useDefault": False,
        ...
        if self.get_event(event_id):
            created_event = (
                self._service.events()
                .patch(eventId=event_id, calendarId=self.calendar_id, body=event)
                .execute()
            )
        else:
            created_event = (
