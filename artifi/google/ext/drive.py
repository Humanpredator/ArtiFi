...
from artifi.config.ext.exception import (
    DriveCloneError,
    DriveDownloadError,
    DriveError,
    DrivePropertiesError,
    DriveUploadError,
)
...
class GoogleDrive(Google):
    def __init__(
        self, context, scope, drive_id, use_sa=False, is_td=False, stop_duplicate=True
    ):
        ...
        self._sa_idx = (
            randrange(len(os.listdir(self._sa_path))) if (self.use_sa) else None
        )
    ...
    def authorize(self):
        ...
        credentials = self.oauth_creds(
            self.scope, service_user=self.use_sa, cname="drive"
        )
    ...
    def switch_service_account(self):
        ...
        self._sa_count += 1
        self._sa_idx += 1
        self.context.logger.info(f"Switching to {self._sa_idx}.json service account")
        self._service = self.authorize()
    ...
    def get_id_by_url(self, link: str):
        ...
        if "folders" in link or "file" in link:
            ...
            if not (res := re.search(regex, link)):
                self.context.logger.info(f"No Folder or File Was Found On Given Link!")
                return None
            return res.group(5)
    ...
    def create_folder(self, directory_name, parent_id):
        ...
        self.context.logger.info(f"Created G-Drive FolderName: {file.get('name')}")
        return file_id
...
class DriveUpload:
    ...
    def on_upload_progress(self):
        ...
        progress = {
            ...
            "speed": f"{readable_size(self.__UPLOADED_BYTES__ / (time.time() + 1 - self.__UPLOAD_STARTED_TIME))}/s",
        }
        return progress
    ...
    def _upload_file(self, file_path, file_name, mime_type, parent_id):
        ...
        media_body = MediaIoBaseUpload(
            fh, mimetype=mime_type, resumable=True, chunksize=10 * 1024 * 1024
        )
        ...
        while True:
            ...
            try:
                ...
                if self.gdrive.use_sa and reason in [
                    ...
                    return self._upload_file(file_path, file_name, mime_type, parent_id)
                else:
        ...
        file = (
            ...
            .get(supportsTeamDrives=True, fileId=file_id)
            .execute()
        )
...
class DriveDownload:
    ...
    def __init__(self, gdrive, drive_link):
        ...
        self._properties = self.gdrive.Properties(self._drive_link)
    ...
    def on_download_progress(self):
        ...
        progress = {
            ...
            "speed": f"{readable_size(self.__DOWNLOADED_BYTES__ / (time.time() + 1 - self.__DOWNLOAD_START_TIME))}/s",
        }
        return progress
...
class DriveProperties:
    ...
    def delete(self, link: str):
        ...
        try:
            res = (
                self.gdrive.service.files()
                .delete(fileId=file_id, supportsTeamDrives=self.gdrive.is_td)
                .execute()
            )
...
class DriveCloner:
    ...
    def on_clone_progress(self):
        ...
        progress = {
            ...
            "speed": f"{readable_size(self.__TRANSFERRED_BYTES / (time.time() + 1 - self.__CLONE_STARTED_TIME))}/s",
        }
        return progress
    ...
    def _copy_file(self, file, dest_id):
        ...
        try:
            drive_file = (
                self.gdrive.service.files()
                .copy(supportsAllDrives=True, fileId=file.get("id"), body=file_metadata)
                .execute()
            )
            self.__TRANSFERRED_BYTES += int(file["size"])
        except HttpError as err:
            reason = err.error_details[0]["reason"]
    ...
    def _clone_folder(self, local_path, file_id, parent_id):
        ...
        for item in files:
            if self.is_cancelled:
                raise DriveCloneError("Cloning Was Cancelled By User!")
            if item.get("mimeType") == self.gdrive.drive_folder_mime:
                self.__TOTAL_FOLDERS += 1
                self.gdrive.context.logger.info(f"Syncing: {item['name']}")
