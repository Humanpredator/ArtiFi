...
class DriveDownload:
    ...
    def _download_file(self, path, file):
        ...
        if os.path.exists(file_path):
            self.gdrive.context.logger.info(f"FileName Already Exists: {new_file_name}")
            return False
    ...
    def download(self, unique=True):
        ...
        path = (
            os.path.join(self.gdrive.context.directory, str(uuid.uuid4()).lower()[:5])
            if unique
            else (self.gdrive.context.directory)
        )
...
class DriveProperties:
    ...
    def stop(self, channel_id, resource_id):
        ...
        try:
            self.gdrive.service.channels().stop(body=body).execute()
    ...
    def watch(self, **kwargs):
        ...
        response = (
            self.gdrive.service.files()
            .watch(fileId=file_id, body=body, supportsTeamDrives=True)
            .execute()
        )
        return response
