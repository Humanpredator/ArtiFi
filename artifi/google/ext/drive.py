...
class GoogleDrive(Google):
    def __init__(
        self, context, scope, drive_id, use_service_acc=False, is_team_drive=False
    ):
    ...
    def _create_folder(self, directory_name, parent_id):
        ...
        file_metadata = {"name": directory_name, "mimeType": self.__folder_mime_type}
    ...
    def _delete_file(self, link: str):
        ...
        try:
            res = (
                self._service.files()
                .delete(fileId=file_id, supportsTeamDrives=self._is_td)
                .execute()
            )
            msg = res
    ...
    def _download_file(self, file_id, path, filename, mime_type):
        ...
        if crm := export_mime.get(mime_type, None):
            request = self._service.files().export(fileId=file_id, mimeType=crm[0])
            new_file_name += crm[1]
        else:
    ...
    def _update_file(self, file_id, file_path):
        ...
        media_body = MediaFileUpload(
            file_path, resumable=True, chunksize=50 * 1024 * 1024
        )
    ...
    def _upload_file(self, file_path, file_name, mime_type, parent_id):
        ...
        # Insert a file
        drive_file = self._service.files().create(
            supportsTeamDrives=True, body=file_metadata, media_body=media_body
        )
        response = None
        ...
        if os.path.isfile(file_folder_path):
            try:
                ...
                link = self._upload_file(
                    file_folder_path, file_name, mime_type, self.parent_id
                )
                if self.is_cancelled:
                    return
                if link is None:
