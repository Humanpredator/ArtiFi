...
class GoogleMail(Google):
    ...
    def list_messages(self, query: str = None, fetch_limit: int = None):
        """
        ...
        while True:
            results = (
                ...
                .messages()
                .list(
                    userId="me", q=query, pageToken=page_token, maxResults=fetch_limit
                )
                .execute()
            )
    ...
    def view_mail(self, msg_id):
        """

        @param msg_id:
        @return:
        """
        message = self._service.users().messages().get(userId="me", id=msg_id).execute()
        return message
